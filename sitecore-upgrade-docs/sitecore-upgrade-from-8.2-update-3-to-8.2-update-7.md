...
We have a decoupled Sitecore solution that is, Sitecore website's all folder/files are reside in a Sitecore.Custom.8.2.xx NuGet package and custom code/component are managed as part of Visual Studio solution. NuGet package is being referenced in Visual Studio solution. 
During build process, extact NuGet package then copies over to the destination folder.
...
- To upgrade the databases on solutions that are running Sitecore 8.2 rev. 161115 (Update-1) to Sitecore Experience Platform 8.2 rev. 170728 (8.2 Update-5)
  - If we are using the Sitecore Experience Database (xDB) functionality, then we must need to execute the SXP82U6_BeforeInstall.sql script for the Reporting database.
